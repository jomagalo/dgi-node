...
func Test_addInvitationReward(t *testing.T) {
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[0]).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(common.Address{0xa1}).Sign())
	...
	require.Equal(t, "1.847474595010569582", ConvertToFloat(appState.State.GetLockedStake(common.Address{0xa2})).String())
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[1]).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[2]).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(common.Address{0xa5}).Sign())
	...
	require.Equal(t, "5147.686539282495524245", ConvertToFloat(appState.State.GetLockedStake(common.Address{0xa6})).String())
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[3]).Sign())
	...
	require.Equal(t, "1677.857398311075623942", ConvertToFloat(appState.State.GetLockedStake(common.Address{0xa7})).String())
	...
	require.Equal(t, "1673.94453215005351672", ConvertToFloat(appState.State.GetLockedStake(common.Address{0xa8})).String())
	...
	require.Zero(t, appState.State.GetLockedStake(common.Address{0xff}).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[4]).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(common.Address{0xa9}).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(common.Address{0xaa}).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[5]).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(common.Address{0xab}).Sign())
	...
	require.Equal(t, "678.899670146385092848", ConvertToFloat(appState.State.GetLockedStake(common.Address{0xac})).String())
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[6]).Sign())
	...
	require.Equal(t, "141.11363524819448702", ConvertToFloat(appState.State.GetLockedStake(common.Address{0xad})).String())
	...
	require.Equal(t, "140.784531668290356371", ConvertToFloat(appState.State.GetLockedStake(common.Address{0xae})).String())
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[7]).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(common.Address{0xaf}).Sign())
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[9]).Sign())
	...
	require.Equal(t, "0.000558281147219847", ConvertToFloat(appState.State.GetLockedStake(common.Address{0xb1})).String())
	...
	require.Zero(t, appState.State.GetLockedStake(addrs[11]).Sign())
	...
	require.Equal(t, "0.000000000000003456", ConvertToFloat(appState.State.GetLockedStake(common.Address{0xb3})).String())
}
